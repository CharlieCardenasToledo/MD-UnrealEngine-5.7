...
> Source: https://dev.epicgames.com/documentation/en-us/unreal-engine/megalights-in-unreal-engine
...
![Image](https://dev.epicgames.com/community/api/documentation/image/35072cc3-e8e6-415f-a2cb-91c263f17cab)
...
![Image](https://dev.epicgames.com/community/api/documentation/image/743b1d3c-d38c-4c66-83d9-e25663db762d)
...
![Image](https://dev.epicgames.com/community/api/documentation/image/f0946efe-b04d-4ad0-956a-4cdf46c9a8e8)
...
![Image](https://dev.epicgames.com/community/api/documentation/image/13db5e47-c170-4035-b48f-f8fa0b56fb96)
...
![Image](https://dev.epicgames.com/community/api/documentation/image/857f4fda-f355-4401-a8de-3e0a1d75345a)
...
![Image](https://dev.epicgames.com/community/api/documentation/image/f7a12747-1eb4-4d01-9bc3-04ac238f6f66)
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "While Virtual Shadow Maps will cast shadows directly from the non-simplified Nanite geometry, it only approximates area shadows. There is CPU, memory and GPU time overhead for VSMs on a per-light basis for preparing shadow map depths in advance.",
      "settings": {
        "is_hidden": false
      }
    }
...
![Image](https://dev.epicgames.com/community/api/documentation/image/5c4234bb-0b2e-491b-9558-aed9e22b0d7b)
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "MegaLights can be disabled per scalability level or device profile with <code>r.MegaLights.Allow 0</code>.",
      "settings": {
        "is_hidden": false
      }
    }
...
- Virtual Shadow Map projection Virtual Shadow Maps can still be used with MegaLights if selected as the shadow method on the individual light actor’s settings.
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Additional visualization tools will be made available as MegaLights is developed into a production-ready feature of Unreal Engine.",
      "settings": {
        "is_hidden": false
      }
    }
...
![Image](https://dev.epicgames.com/community/api/documentation/image/f3b6688d-6bc2-42ab-9314-108897ffbd9c)
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "To minimize noise avoid placing lights inside geometry, optimize light attenuation range, spotlight cone angles, and rect light barn doors to narrow down the influence of light.",
      "settings": {
        "is_hidden": false
      }
    }
...
|   |   |
| --- | --- |
| ![Image](https://dev.epicgames.com/community/api/documentation/image/aa674e69-c318-436f-a097-d51b55a4207b) | ![Image](https://dev.epicgames.com/community/api/documentation/image/68b2f309-1cf9-4b7d-b76b-df9e0bd77a63) |
...
![Image](https://dev.epicgames.com/community/api/documentation/image/d59dd3b4-ecaf-4c00-9f37-4d4b79c87f45)
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Nanite Fallback Mesh triangle count and number of instances included in the Ray Tracing Scene will affect ray tracing BVH build times, used memory, and ray tracing performance. We recommend carefully increasing them according to the available performance and memory budget of your project.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "For traces that extend past the ray tracing scene coverage (or when the far field representation is too coarse), MegaLights supports distant screen traces. The length of the distant screen traces is defined as a percentage of the screen using <code class=\"inline-code\">r.MegaLights.DistantScreenTraces.Length</code>.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Changing these settings will affect the quality of shadowing on both lit translucency and volumetric fog.",
      "settings": {
        "is_hidden": false
      }
    }
...
![Niagara emitter with MegaLights settings.](https://dev.epicgames.com/community/api/documentation/image/a2935c53-1c16-45dd-aab2-07b55b9a337e)

_Niagara emitter with MegaLights settings._

1. Select the **Light Renderer** module under the emitter.
2. In the **Details** panel, enable **Allow Mega Lights** and **Mega Lights Cast Shadows**. Checking the second setting makes the particle lights cast shadows. Checking the first setting enables MegaLights for the lights spawned.
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Since an emitter can spawn a lot of lights, care needs to be taken to avoid having too many strong lights affecting a screen pixel at the same time. Otherwise, quality may degrade noticeably. We recommend keeping the lights small, sparse in space, and only enable shadow casting on a small number of particle lights. ",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "For scalability of lighting for volumetric fog and hair strands, you can use console variables under <code class=\"inline-code\">r.megalights.volume.*</code> to adjust lighting quality for them.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "MegaLights doesn’t yet have dedicated support for <a href=\"animating-characters-and-objects\\Sequencer\\movie-render-pipeline#movierenderqueue\">Movie Render Queue</a>, but good results can be achieved with either using Temporal Super Resolution (TSR) as anti-aliasing method or setting the Temporal Sample Count to around 8, which is able to resolve lighting correctly.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "MegaLights cost is mostly constant, and there isn’t a large difference between unshadowed and shadowed lights, so all lights in the scene can have shadows enabled.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Ideally, MegaLights should be used with <a href=\"building-virtual-worlds\\lighting-and-shadows\\global-illumination\\lumen\\TechOverview\\#hardwareraytracing\">Lumen HWRT</a>, which allows sharing Ray Tracing Scene overhead and optimizations between both systems.",
      "settings": {
        "is_hidden": false
      }
    }
...
```json
{
  ...
  "blocks": [
    {
      "type": "paragraph",
      "content": "Unreal Engine uses Async Compute to overlap multiple dispatches from various features. <code>Stat GPU</code> and <code>ProfileGPU</code> timings will be distorted until you disable Async Compute using the <code>r.RDG.AsyncCompute 0</code> console command.",
      "settings": {
        "is_hidden": false
      }
    }
