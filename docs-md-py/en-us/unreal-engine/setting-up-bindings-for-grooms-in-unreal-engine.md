...
> Source: https://dev.epicgames.com/documentation/en-us/unreal-engine/setting-up-bindings-for-grooms-in-unreal-engine
...
The binding data is only used if the current level of detail (LOD) **Binding Type** is set to **Skinning**. This can be set up separately for each LOD in the [Groom Asset Editor](https://dev.epicgames.com/documentation/en-us/unreal-engine/groom-asset-editor-user-guide-in-unreal-engine) under the **LOD** panel settings. You can verify the current active binding type in the level by selecting **Lit > Instances** in the viewport menus and visualizing the Binding Type for each.
